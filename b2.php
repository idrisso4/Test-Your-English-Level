...
$_SESSION['dict']=$d;
  ...
  if ($a>23 && $b>23 && $c>23 && $d>23) {
    $requete="UPDATE users SET level='c1' WHERE cin='$cin'";
    $result=mysql_query($requete,$idcon);
    $_SESSION['level']="c1";
    if (!$result) {
      echo "erreur de modification";
...
<BODY class="bod">
  ...
  <label><h2 align="center" style="margin-top: 14%;">Test for lelvel C1</h2></label> 
  <form name="b2" method="post" action='<?= $_SERVER["PHP_SELF"] ?>' enctype="application/x-www-form-urlencoded">            
